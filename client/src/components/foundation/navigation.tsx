  ...
  return (
    <header className="border-b border-border bg-white">
      <div className="portal-container flex h-[72px] items-center justify-between">
        <a aria-label="Recruitment Portal UI kit" className="flex items-center gap-2.5 text-primary" href="/ui-kit">
          <AlignmentMark />
        ...
        <nav aria-label="Public navigation" className="hidden items-center gap-6 md:flex">
          <button className="text-sm font-medium text-muted-foreground transition-colors hover:text-primary" onClick={() => handlePlaceholder("Home")} type="button">Home</button>
          <button className="text-sm font-medium text-muted-foreground transition-colors hover:text-primary" onClick={() => handlePlaceholder("How It Works")} type="button">How It Works</button>
          <FoundationButton onClick={() => handlePlaceholder("Start Application")} size="md">Start Application</FoundationButton>
        </nav>
      ...
      {open ? (
        <div className="border-t border-border bg-white px-4 py-3 shadow-[0_10px_20px_rgba(16,24,40,0.07)] motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-top-2 motion-safe:duration-200 md:hidden">
          <nav aria-label="Mobile public navigation" className="portal-container flex flex-col gap-1 px-0">
            <button className="rounded-md px-3 py-2.5 text-left text-sm font-medium text-foreground hover:bg-portal-surface" onClick={() => handlePlaceholder("Home")} type="button">Home</button>
            <button className="rounded-md px-3 py-2.5 text-left text-sm font-medium text-foreground hover:bg-portal-surface" onClick={() => handlePlaceholder("How It Works")} type="button">How It Works</button>
            <FoundationButton className="mt-2 w-full" onClick={() => handlePlaceholder("Start Application")}>Start Application</FoundationButton>
          </nav>
        </div>
      ) : null}
    </header>
