...
export function ApplicationSummary({ activeStep, submitted = false }: { activeStep: number; submitted?: boolean }) {
  return (
...
export function ApplicationShell({ activeStep, children, showSummary = false, submitted = false }: { activeStep: number; children: ReactNode; showSummary?: boolean; submitted?: boolean }) {
  return (
    <div className="min-h-screen bg-portal-surface text-foreground">
      ...
      <main className="mx-auto max-w-[1000px] px-4 py-10 sm:px-6 sm:py-12">{showSummary ? <div className="grid gap-10 lg:grid-cols-[minmax(0,69fr)_minmax(250px,31fr)] lg:gap-12"><div>{children}</div><div className="hidden lg:block"><ApplicationSummary activeStep={activeStep} submitted={submitted} /></div></div> : children}</main>
    </div>
  );
}
