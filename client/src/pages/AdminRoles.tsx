...
export default function AdminRoles() {
  ...
  const visibleRoles = useMemo(() => roles.filter((role) => (!normalisedSearch || [role.title, role.department, role.location].some((value) => value.toLowerCase().includes(normalisedSearch))) && (statusFilter === "all" || role.status === statusFilter)), [normalisedSearch, roles, statusFilter]);
  const hasActiveFilters = Boolean(normalisedSearch) || statusFilter !== "all";
