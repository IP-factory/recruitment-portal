...
function NavigationList({ location, navigate, close }: { location: string; navigate: (href: string) => void; close?: () => void }) {
  const itemClass = (href: string) => `flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors ${location === href ? "bg-portal-blue-soft text-primary" : "text-muted-foreground hover:bg-portal-surface hover:text-primary"}`;
