...
export default function ApplicantAssessmentComplete() {
  ...
  const reviewResponses = () => { const state = loadAssessmentResponseState(); saveAssessmentResponseState({ ...state, currentQuestionIndex: 0 }); setLocation("/apply/business-development-officer/assessment/questions"); };
  return (
    <ApplicationShell activeStep={2}>
      <section className="mx-auto max-w-[620px] py-10 text-center sm:py-14">
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:justify-center"><FoundationButton onClick={() => setLocation("/apply/business-development-officer/review")} size="lg">Continue to review</FoundationButton><FoundationButton onClick={reviewResponses} variant="secondary">Review assessment responses</FoundationButton></div>
      </section>
    </ApplicationShell>
  );
}
