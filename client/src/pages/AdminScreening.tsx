...
type SortOption = "final-score-desc" | "final-score-asc" | "application-desc" | "application-asc" | "candidate";
...
export default function AdminScreening() {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      ...
      if (sort === "final-score-desc") {
        const av = a.finalScore ?? -1;
        const bv = b.finalScore ?? -1;
        if (bv !== av) return bv - av;
        return new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime();
      }
      if (sort === "final-score-asc") {
        const av = a.finalScore ?? 999;
        const bv = b.finalScore ?? 999;
  ...
  return <AdminShell title="Screening">
    <section><p className="text-[12px] font-medium text-muted-foreground">Candidate review</p><h2 className="mt-2 text-3xl font-semibold tracking-[-0.035em] text-primary">Candidate Screening</h2><p className="mt-2 max-w-3xl text-[15px] leading-6 text-muted-foreground">Review real candidates sorted by Final Score. Shortlist, hold or close manually. All actions persist in TiDB.</p></section>
    ...
    <section className="mt-6 rounded-xl border border-border bg-white p-4 shadow-none sm:p-5 lg:p-6">
      <div className="flex flex-col gap-5 border-b border-border pb-5 xl:flex-row xl:items-end xl:justify-between">
        <div className="grid gap-3 sm:grid-cols-2 xl:min-w-[550px]">
          <div><label className="mb-1.5 block text-[12px] font-medium text-muted-foreground" htmlFor="screening-sort">Sort by</label><FoundationSelect id="screening-sort" onChange={(e) => setSort(e.target.value as SortOption)} value={sort}><option value="final-score-desc">Final Score — highest first</option><option value="final-score-asc">Final Score — lowest first</option><option value="application-desc">Application date — newest first</option><option value="application-asc">Application date — oldest first</option><option value="candidate">Candidate name</option></FoundationSelect></div>
      ...
      {filteredRecords.length ? <>
        <div className="mt-3 hidden overflow-x-auto md:block"><table className="w-full min-w-[1100px] text-left"><thead className="border-b border-border"><tr>
          ...
          <th className="px-3 py-3 text-[11px] font-semibold uppercase tracking-[0.08em] text-muted-foreground">Final Score</th>
